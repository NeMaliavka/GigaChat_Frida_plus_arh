# # app/handlers/utils/booking_utils.py
# import logging
...
async def show_available_dates(event: Union[Message, CallbackQuery], state: FSMContext):
    ...
    try:
        ...
        # --- ИЗМЕНЕНИЕ 1: Используем новый режим форматирования для кнопок ---
        # Это уберет лишнее время "00:00" с кнопок выбора дня.
        date_buttons = [
            [InlineKeyboardButton(
                text=format_date_russian(datetime.strptime(date_str, '%Y-%m-%d'), 'date_only'), 
